...
组函数又称为聚合函数
  输入多个值，最终只会返回一个值
  组函数仅可用于选择列表或查询的having子句,不可以和where一起使用
单行函数
  输入一个值，输出一个值
*/
--查询所有员工的薪水总和
...
--sign:获取正负值,若为正值返回1，负值返回-1，0返回0
select sign(100) from dual;
--power:x的y次幂
select power(2,3) from dual;

--日期函数
--sysdate/current_date:返回系统当前日期
...
/*
转换函数  
     oracle中存在数值的隐式转换和显式转换
     隐式转换指的是字符串可以转换为数值或日期
显式转换：
     to_char:当由数值或者日期转换为字符串的时候，必须要规定格式
*/
...
--number:to_char
select to_char(123.456789,'9999') from dual; --' 123'
select to_char(123.456789,'0000') from dual; --'0123'
...
count():记录数，处理的时候会跳过空值而处理非空值
    count一般用来获取表中的记录条数，获取的时候可以使用*或者某一个具体的类，甚至可以使用纯数字来代替，
    但是从运行角度考虑，建议使用数字或者某一个具体的列而不要用*
sum():求和，只适用于数值类型的数据
*/
...
select sum(sal) from emp;
--group by：按照某些相同的值进行分组操作
--group by进行分组操作的时候，可以指定一个列或者多个列，但是当使用了group by之后，选择列表中只能包含组函数的值或者group by的普通字段
--求每个部门的平均薪水
...
--select选择列表中包含的ename，既不是组函数的值也不是group by的普通字段(此处group by的普通字段是deptno)
select avg(sal),ename from emp group by deptno having avg(sal) > 2000;
--获取表的记录条数，结果都是14
select count(*) from emp;
