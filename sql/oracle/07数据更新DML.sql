...
--实际项目中，使用最多的是读取操作，但是插入数据和删除数据同等重要，而修改操作相对较少
...
--复制表结构，但是不复制表数据(因为1不等于2,所以查询不到结果)，不会复制约束
...
--整张表的数据全部清空
delete from emp2;
--truncate:也可以清空数据，与delete有所不同，delete在进行删除的过程中经过事务，而truncate不经过事务，一旦删除就是永久删除，不具备回滚的操作
--效率比较高，但是容易发生误操作，所以不建议使用
truncate table emp2;

/*
修改操作：
    语法：update tablename set col = val1,col2 = val2 where condition;
    可以更新或修改满足条件的一个列或者多个列
*/
--更新单列
update emp set ename = 'heihei' where ename = 'hehe';
--更新多个列的值
update emp set job = 'teacher',mgr=7902 where empno = 7;

/*
总结：
增删改是数据库的常用操作，在进行操作的时候都需要《事务》的保证，也就是说每次在pl/sql中执行sql语句之后都需要完成commit
事务变得非常关键：最主要的目的是保证数据的一致性。
       如果同一份数据，在同一个时刻，只能有一个人访问就不会出现数据错乱的问题，但是在现在的项目中，更多的是并发访问
