...
--视图的使用
select * from v_emp;
--向视图中添加数据,执行成功后，需要提交事务。绿色表示提交事务，让数据生效；红色表示回滚事务，让数据恢复原状态
insert into v_emp(empno,ename) values(1111,'zhangsan');--执行后，这条数据就已经在缓存空间中的，即使没有提交，也是能查询出来的；而提交事务就是要把缓存空间的数据刷写到文件中，使其永久生效
select * from emp; --没有提交事务时，也是能查出来刚刚添加的数据，但是只在当前会话生效
--如果定义的视图是非只读视图的话，可以通过视图向表中插入数据；如果是只读视图，则不可以插入数据
create view v_emp2 as select * from emp with read only;
select * from v_emp2;
--insert语句无法执行成功，提示ORA-42399:无法对只读视图执行DML操作
...
--练习：求平均薪水的等级最低的部门，它的部门名称是什么，完全使用子查询
--1、先求平均薪水
       ...
       (select min(t.gd)
          from (select t.deptno, sg.grade gd
                  ...
                  join (select e.deptno, avg(e.sal) vsal
                        ...
                        group by deptno) t
                    on t.vsal between sg.losal and sg.hisal) t);
--查看sql语句能发现，sql中有很多的重复sql子查询，可以通过视图将重复的语句抽象出来
--创建视图
